...
class HuxiuSpider(Spider):
    ...
    def parse(self, response):
        ...
        title_big = sel.xpath("//div[@class='big2-pic-right pull-right']/div[1]/a/text()").extract()
        href_big = sel.xpath("//div[@class='big2-pic-right pull-right']/div[1]/a/@href").extract()
        yield Request(url=href_big, callback=self.parse_item, meta={'title':title_big})
        post_list = sel.xpath("//div[@class='mod-info-flow']/div").extract()
        for post in post_list:
            post = Selector(text=post)
            title = post.xpath("//div[@class='mod-info-flow']/div[1]/div[@class='mob-ctt']/h3/text()").extract()[0]
            href = 'http://www.huxiu.com' + post.xpath("//div[@class='mod-info-flow']/div[1]/div[@class='mob-ctt']/h3/a/@href").extract()[0]
